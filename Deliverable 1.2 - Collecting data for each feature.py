##################  Collecting data for each feature  ##################
...
df.drop(['Date_of_policy','Claim_before_Premium_Paid','Customer_Locality','Customer_Region','Claim_Description','charges'], axis = 1, inplace=True)
...
oc=[]   #Outstanding charges
for i in range(0,1000):
    i=round(random.uniform(0.01,0.08), 3) #Need to confirm the outstanding charges % , considering 1-6%
    oc.append(i)
df1=pd.DataFrame(oc)
df1.columns=["Percentage1"]
df1["Outstanding charges"]=0
df1["Outstanding charges"] = df1["Percentage1"]*df["Premium_price_"]
df["Outstanding_charges"]=df1["Outstanding charges"] 
...
df.to_csv(r'C:\360\Data_Prepared.csv', index=False)
